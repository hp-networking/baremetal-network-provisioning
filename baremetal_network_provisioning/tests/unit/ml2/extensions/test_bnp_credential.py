...
class TestBnpCredential(test_plugin.NeutronDbPluginV2TestCase,
    ...
    def _create_credentials_for_snmp(self, req, name, dbval, protoval, body):
        create_req = self.new_create_request('bnp-credentials', req,
                                             'json')
        with contextlib.nested(
            mock.patch.object(validators, 'validate_access_parameters',
                              return_value=protoval),
            mock.patch.object(self.bnp_wsgi_controller,
                              '_create_snmp_creds',
                              return_value=self._create_snmp_cred
                              (body, name, dbval, protoval))):
    ...
    def _create_snmp_cred(self, body, ret, dbval, protoval):
        ...
        with contextlib.nested(
            ...
            return self.bnp_wsgi_controller._create_snmp_creds(fake_context,
                                                               body, protoval)

    def _create_credentials_for_netconf(self, req, name, dbval, protoval,
                                        body):
        create_req = self.new_create_request('bnp-credentials', req,
                                             'json')
        with contextlib.nested(
            mock.patch.object(validators, 'validate_access_parameters',
                              return_value=protoval),
            mock.patch.object(self.bnp_wsgi_controller,
                              '_create_netconf_creds',
                              return_value=self._create_netconf_cred
                              (body, name, dbval, protoval))):
            self.bnp_wsgi_controller.create(create_req)
    ...
    def _create_netconf_cred(self, body, ret, dbval, protoval):
        ...
        with contextlib.nested(
            ...
            return self.bnp_wsgi_controller._create_netconf_creds(fake_context,
                                                                  body,
                                                                  protoval)

    def test_create_snmp_credentials_with_existing_name(self):
        self._create_credentials_for_snmp(self.snmp_req1, None,
                                          self.dbval, self.protoval,
                                          self.snmp_body1)
        self.assertRaises(webob.exc.HTTPConflict,
                          self._create_credentials_for_snmp,
                          self.snmp_req1, '123', self.dbval,
                          self.protoval, self.snmp_body1)

    def test_create_netconf_credentials_with_existing_name(self):
        self._create_credentials_for_netconf(self.netconf_req1, None,
                                             self.dbval1,
                                             self.protoval1,
                                             self.netconf_body1)
        self.assertRaises(webob.exc.HTTPConflict,
                          self._create_credentials_for_netconf,
                          self.netconf_req1, '123', self.dbval1,
                          self.protoval1, self.netconf_body1)
    ...
    def test_create_snmp_credentials_with_invalid_protocol(self):
        ...
        self.assertRaises(webob.exc.HTTPBadRequest,
                          self._create_credentials_for_snmp,
                          req2, None, self.dbval, self.protoval,
                          self.snmp_body1)
    ...
    def test_create_netconf_credentials_with_invalid_protocol(self):
        ...
        self.assertRaises(webob.exc.HTTPBadRequest,
                          self._create_credentials_for_netconf,
                          req2, None, self.dbval1, self.protoval1,
                          self.netconf_body1)
